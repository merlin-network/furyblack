...
## default port prefixes for fury
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/fury keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/fury init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/fury add-genesis-account \
    "$("$PWD"/build/fury keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000afury,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/fury gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/fury collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/fury validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    highbury_PID=$!
    echo "started fury node, pid=$highbury_PID"
    # add PID to array
    arr+=("$highbury_PID")
