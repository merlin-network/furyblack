...
LEDGER_ENABLED ?= true
BINDIR ?= $(GOPATH)/bin
highbury_BINARY = fury
highbury_DIR = fury
BUILDDIR ?= $(CURDIR)/build
SIMAPP = ./app
HTTPS_GIT := https://github.com/merlin-network/fury-fury.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=fury \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(highbury_BINARY) \
...
build-docker:
	...
	$(DOCKER) create --name fury -t -i ${DOCKER_IMAGE}:latest fury
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/merlin-network/fury-fury/types"
	godoc -http=:6060
...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.$(highbury_DIR)/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.$(highbury_DIR)/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.$(highbury_DIR)/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.$(highbury_DIR)/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/docs/statik/statik.go" -not -name '*.pb.go' | xargs goimports -w -local github.com/merlin-network/fury-fury
.PHONY: format
...
ETHERMINT_URL      	= https://raw.githubusercontent.com/Fury-Network/ethermint-v2/v0.10.0
...
ifeq ($(OS),Windows_NT)
	...
	IF not exist "build/node0/$(highbury_BINARY)/config/genesis.json" docker run --rm -v $(CURDIR)/build\fury\Z fury/node "./fury testnet --v 4 -o /fury --keyring-backend=test --ip-addresses furynode0,furynode1,furynode2,furynode3"
	docker-compose up -d
else
	mkdir -p localnet-setup
	@$(MAKE) localnet-build

	if ! [ -f localnet-setup/node0/$(highbury_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/localnet-setup:/fury:Z fury/node "./fury testnet --v 4 -o /fury --keyring-backend=test --ip-addresses furynode0,furynode1,furynode2,furynode3"; fi
	docker-compose up -d
endif
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\localnet-setup\node0\fury:fury\Z fury/node "./fury unsafe-reset-all --home=/fury"
	@docker run --rm -v $(CURDIR)\localnet-setup\node1\fury:fury\Z fury/node "./fury unsafe-reset-all --home=/fury"
	@docker run --rm -v $(CURDIR)\localnet-setup\node2\fury:fury\Z fury/node "./fury unsafe-reset-all --home=/fury"
	@docker run --rm -v $(CURDIR)\localnet-setup\node3\fury:fury\Z fury/node "./fury unsafe-reset-all --home=/fury"
else
	@docker run --rm -v $(CURDIR)/localnet-setup/node0/fury:/fury:Z fury/node "./fury unsafe-reset-all --home=/fury"
	@docker run --rm -v $(CURDIR)/localnet-setup/node1/fury:/fury:Z fury/node "./fury unsafe-reset-all --home=/fury"
	@docker run --rm -v $(CURDIR)/localnet-setup/node2/fury:/fury:Z fury/node "./fury unsafe-reset-all --home=/fury"
	@docker run --rm -v $(CURDIR)/localnet-setup/node3/fury:/fury:Z fury/node "./fury unsafe-reset-all --home=/fury"
endif
