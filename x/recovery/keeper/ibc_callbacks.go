...
import (
	...
	"github.com/merlin-network/fury/v6/ibc"
	fury "github.com/merlin-network/fury/v6/types"
...
//  - sends over all fury native tokens
...
func (k Keeper) OnRecvPacket(
	...
	// Get addresses in `fury1` and the original bech32 format
	...
	// a sender's own account on fury (sender == recipient)
	...
	if account != nil && fury.IsSupportedKey(account.GetPubKey()) {
		return ack
	}
	...
	k.bankKeeper.IterateAccountBalances(ctx, recipient, func(coin sdk.Coin) (stop bool) {
		...
		err = k.transferKeeper.SendTransfer(
			...
			recipient,                 // recipient is the address in the fury chain
...
// the IBC denomination, i.e port and channel on fury for the voucher. It
// returns an error if:
